...
test.describe('Pet Contract Validation Suite', () => {
  test('Response schema matches expected OpenAPI contract', async ({ petService }) => {
    test.info().annotations.push({ type: 'epic', description: 'Contract Testing' });
    test.info().annotations.push({ type: 'feature', description: 'Pet API Contract' });
    test.info().annotations.push({ type: 'story', description: 'Schema Validation - Pet' });
    test.info().annotations.push({ type: 'severity', description: 'normal' });
    
    test.info().annotations.push({ type: 'description', description: 'Validates that the Pet API response schema matches the OpenAPI specification contract.' });
